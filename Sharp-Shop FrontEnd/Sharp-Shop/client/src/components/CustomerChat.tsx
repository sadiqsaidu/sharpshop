      ...
      >
        {!isOpen && (
          <Button
            ...
            size="icon"
            className="h-14 w-14 rounded-full bg-yellow-400 hover:bg-yellow-500 text-black shadow-lg"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[90vw] max-w-[400px] h-[600px] max-h-[80vh] bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
              ...
              <div className="space-y-4">
                 ...
                 <div className="flex gap-3">
                    <Avatar className="h-8 w-8 mt-1 border border-white/10">
                        <AvatarFallback className="bg-yellow-400 text-black text-xs">AI</AvatarFallback>
                    </Avatar>
                ...
                <div className="bg-[#1a1a1a] border-t border-white/10 p-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
                    {products.map(p => (
                        <div key={p.id} className="inline-block w-32 mr-2 bg-[#222] rounded-lg p-2 border border-white/5 align-top">
                            ...
                            <p className="text-yellow-400 text-xs text font-bold">₦{p.price.toLocaleString()}</p>
                        </div>
                    ))}
                </div>
            )}
            ...
            <div className="p-3 bg-[#222] border-t border-white/10 flex gap-2">
              <Input
                ...
                className="bg-[#121212] border-white/10 text-white placeholder:text-white/30 focus-visible:ring-yellow-400"
              />
              <Button 
                onClick={handleSend}
