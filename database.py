...
def get_or_create_trader(whatsapp_number: str, business_name: str = "New Seller") -> dict:
    ...
    print(f"🔍 Looking for trader with WhatsApp number: {normalized_number}")
    ...
    if normalized_number.startswith("+"):
        ...
        if result.data and len(result.data) > 0:
            print(f"✅ Found existing trader (without +) for {alt_number}: {result.data[0]['business_name']}")
            return result.data[0]
    else:
        # Try with + prefix
        ...
        result = supabase.table("traders").select("*").eq("whatsapp_number", alt_number).execute()
        if result.data and len(result.data) > 0:
            print(f"✅ Found existing trader (with +) for {alt_number}: {result.data[0]['business_name']}")
            return result.data[0]
